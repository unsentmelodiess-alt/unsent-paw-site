...
export function ListeningRail({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <aside className={`hidden rounded-full border px-2 py-4 shadow-lg lg:flex lg:flex-col lg:items-center lg:gap-5 ${dark ? "border-white/15 bg-[#202520]/85 text-[#f5f0e8]" : "border-[#d1c7b6] bg-[#faf6ee]/88 text-[#3a332b]"}`} aria-label="Listening rail">
      <span className="writing-mode text-[0.58rem] font-bold uppercase tracking-[.22em] text-[#75836D]">Now listening</span>
      <button className={`grid size-9 place-items-center rounded-full ${dark ? "bg-[#e5b17c] text-[#302a23]" : "bg-[#75836D] text-white"}`} aria-label="Play sample"><Play className="ml-0.5 size-3 fill-current" /></button>
      ...
      <span className="writing-mode text-[0.55rem] font-bold uppercase tracking-[.2em] opacity-60">00:48</span>
    </aside>
  );
}
